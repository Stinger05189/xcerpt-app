...
## 9. LLM Integration & Copilot Automation (v2.1+)

- **IPC Handler Decoupling (`executeLLMComplete`):** In Electron `main.cjs`, never attempt to access private, undocumented handler structures (e.g. `ipcMain.handlers.get(...)`). Extract business logic into standalone asynchronous functions that can be called directly by both primary IPC handlers and internal test connection routines.
- **Chromium Global Sandbox Isolation (`process`):** In renderer process code (`src/`), never access Node.js `process` globals directly without runtime existence guards. All runtime information (versions, OS) should be projected through typed IPC APIs (`window.api.getVersion()`) or rendered as static platform strings.
