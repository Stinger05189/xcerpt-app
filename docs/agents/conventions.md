...
- **$O(\text{depth})$ Ancestor Prefix Lookups:** Never evaluate directory exclusions using linear string loops or regex ignore arrays over thousands of files. Split target paths at slashes and query a Set of ancestor directory prefixes ($O(\text{depth})$ operations, typically $\le 4$).
- **Wildcard Segregation for Ignore Engines:** Only patterns containing true glob wildcards (`*`, `?`, `[`) should be added to `ignore()`. Literal paths and directories must be resolved via $O(1)$ and $O(\text{depth})$ Set checks.
- **Hierarchical Rule Compaction:** Prevent unbounded rule accumulation by running `compactRules()` on mutations. If an ancestor directory is excluded, redundant child file rules must be pruned.
- **Inclusion Whitelist Precedence:** When evaluating file status, an explicit child inclusion rule (`includeExact`) MUST take precedence over inherited ancestor folder exclusions, enabling granular "exclude folder but include this one specific file" curation workflows.
