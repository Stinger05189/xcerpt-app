...
- **Packaged Asset Resolution (Native Images):** When creating native images for IPC (e.g., OS drag-and-drop icons via `nativeImage.createFromPath`), `__dirname` resolves differently in Vite production builds. You must explicitly map the path conditionally: `app.isPackaged ? path.join(__dirname, 'dist', 'icon.png') : path.join(__dirname, 'public', 'icon.png')`.

## 9. Layouts & Data Tables
