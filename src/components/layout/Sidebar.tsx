...
export function Sidebar() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Don't trigger if the user is typing inside an input field (like the blacklist rule input)
      if (document.activeElement?.tagName === 'INPUT' || document.activeElement?.tagName === 'TEXTAREA') {
        return;
      }
      
      if (e.key === 'Tab') {
        e.preventDefault(); // Prevent standard UI focus shifting
  ...
  return (
    <>
      {/* Dimmed backdrop to close sidebar when clicking off it */}
      {isSidebarOpen && (
        <div 
      ...
      <aside className={`fixed top-10 bottom-0 left-0 w-80 bg-bg-panel border-r border-border-subtle flex flex-col z-40 transition-transform duration-300 shadow-2xl ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
